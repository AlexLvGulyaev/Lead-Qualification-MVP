...
```mermaid
flowchart TB
    T["Traefik<br/>(n8n_traefik_1)<br/>/opt/n8n/dynamic.yml<br/>(внешний, не в LQ repo)"] --> C["Client UI<br/>(nginx)"]
    T --> A["Admin UI<br/>(nginx)"]
    T --> N["n8n"]
    A --> B["Admin Backend<br/>(FastAPI)"]
    C --> P["PostgreSQL<br/>(две базы)"]
    B --> P
    N --> P
```
