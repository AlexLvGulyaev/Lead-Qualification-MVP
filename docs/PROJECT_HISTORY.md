...
Проект начинался как ответ на востребованное направление: автоматизация обработки входящих лидов на связке n8n + AI — типовое требование заказов на фриланс-площадках (рыночное подтверждение — в [SPEC.md](SPEC.md), §4).
...
```mermaid
flowchart TB
    C["contacts<br/>(люди/организации)"] -->|1:N| L["leads<br/>(обращения)"] -->|1:N| M["messages<br/>(сообщения)"] -->|1:N| Q["qualifications<br/>(результаты AI)"]
```
