...
**Дата:** 2026-09-15
**Статус:** Администрирование системы: Admin Console, n8n workflows, мониторинг, troubleshooting.
...
Вход в консоль — по Bearer-токену. Токены задаются переменными окружения `LQ_ADMIN_TOKEN` / `LQ_ADMIN_DEMO_TOKEN` (см. `infra/.env.example`; значения не публикуются):

| Токен | Роль | Назначение |
|-------|------|------------|
| **Admin-токен** (`LQ_ADMIN_TOKEN`) | `admin` | Полный доступ к консоли |
| **Demo-токен** (`LQ_ADMIN_DEMO_TOKEN`) | `demo` | Публичный read-only демо-вход (кнопка «Войти в демо-режим») |

Поведение авторизации (`backend/app/api/auth.py`):

- Нет заголовка `Authorization` → **401**;
- Токен неверный → **403**;
- Токены не настроены → авторизация не активна;
- `GET /api/admin/auth/whoami` возвращает роль текущего входа;
- Каждый вход фиксируется в журнале аудита (`audit_logs`).

Роль `demo` и `admin` различаются только в журнале аудита: вся консоль работает в режиме просмотра (read-only), изменения лида и статусы выполняются в Kommo.
...
## ⚙️ 3. Архитектура обработки лидов
